            ...
            <div class="nav-tabs-custom">
              ...
              <div class="tab-content">
                ...
                if (isset($_POST['supp_update']))
                {
                  ...
                  $supp_profile1 = $conn->prepare("SELECT * FROM t_suppin WHERE a_mail= ? AND a_pwd= ?");
                  $supp_profile1->bind_param('ss', $supp_email, $supp_pwd);

                  if($supp_profile1->execute())
                  {
         
                    $supp_update = $conn->prepare("UPDATE t_suppin SET a_name = ?, a_loc = ?");
                    $supp_update->bind_param('ss', $supp_name, $supp_loc);
...
<!-- ---------------------------------------------------------------------------------------------------- -->                  
              ...
              <div class="tab-pane" id="pwd_setting">
                ...
                {
                  $supp_id = clean($_GET['supp_id']);
                  $supp_profile = $conn->prepare("SELECT * FROM t_suppin WHERE a_mail= ?");
                  $supp_profile->bind_param('s', $supp_id);
                  $supp_profile->execute();
                  $supp_profileResult = $supp_profile->get_result();
                ...
                if (isset($_POST['supp_pwd_update']))
                {
                  ...
                  $supp_profile1 = $conn->prepare("SELECT * FROM t_supplier WHERE s_mail= ?");
