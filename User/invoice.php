...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    {
                $user_id =  clean($_POST['inv_id']);
                $user_det = $conn->prepare("SELECT * FROM t_user WHERE u_id= ?");
                $user_det->bind_param('s', $user_id);
                $user_det->execute();
                $user_detResult = $user_det->get_result();
                while ($row = mysqli_fetch_array($user_detResult)) 
                    {
...
?>
            ...
            $cart_product = $conn->prepare("SELECT * FROM t_cart WHERE u_id = ?");
            $cart_product->bind_param('s', $user_id);
            $cart_product->execute();
            $cart_productResult = $cart_product->get_result();
            while ($row1 = mysqli_fetch_array($cart_productResult)) 
